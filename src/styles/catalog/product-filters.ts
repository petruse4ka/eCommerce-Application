import { CHECKBOX_STYLE } from '@/styles/inputs/inputs';

export const FILTERS_STYLES = {
  CONTAINER: ['flex', 'flex-col', 'gap-6'],
  FILTER_CONTAINER: ['flex', 'flex-col', 'gap-2'],
  FILTER_TITLE: ['text-lg', 'font-semibold'],
  OPTIONS_CONTAINER: ['flex', 'flex-col', 'gap-2'],
  OPTION_CONTAINER: ['flex', 'items-center', 'gap-2'],
  CHECKBOX: CHECKBOX_STYLE,
  LABEL: ['text-sm'],
  HIDDEN: ['hidden'],
  DROPDOWN: [
    'bg-white',
    'border',
    'border-black/55',
    'rounded-lg',
    'px-4',
    'py-2',
    'text-dark',
    'text-sm',
    'cursor-pointer',
    'hover:border-black',
    'focus:outline-none',
    'focus:border-black',
    'w-full',
    'sm:w-auto',
  ],
  RANGE_CONTAINER: ['flex', 'items-center', 'gap-2'],
  RANGE_INPUT: [
    'w-20',
    'px-2',
    'py-1',
    'border',
    'border-black/55',
    'rounded',
    'text-sm',
    'hover:border-black',
    'focus:outline-none',
    'focus:border-black',
  ],
  RANGE_LABEL: ['text-sm', 'text-gray-600'],
  SELECTED_FILTERS_CONTAINER: ['flex', 'flex-col', 'gap-2', 'p-4', 'bg-gray-100', 'rounded'],
  FILTER_TAGS: ['flex', 'flex-wrap', 'gap-2'],
  FILTER_ITEM: [
    'flex',
    'items-center',
    'gap-1',
    'px-2',
    'py-1',
    'bg-white',
    'border',
    'rounded',
    'text-sm',
  ],
  FILTER_ITEM_VALUE: ['text-dark'],
  FILTER_LIST: ['flex', 'flex-col', 'gap-2'],
  FILTER_LIST_TITLE: ['text-sm', 'font-semibold', 'text-gray-700'],
  FILTERS_CONTAINER: ['flex', 'flex-col', 'gap-4'],
  FILTERS_TITLE: ['text-lg', 'font-semibold', 'mb-2'],
};
