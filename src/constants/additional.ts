import bicycle from '@/assets/images/additional/delivery-bicycle.png';
import letter from '@/assets/images/additional/letter.png';
import pickup1 from '@/assets/images/additional/pickup-primary.png';
import pickup2 from '@/assets/images/additional/pickup-secondary.png';
import type { additionalPagesData } from '@/types/interfaces';

export const DELIVERY: additionalPagesData = {
  TITLE: 'Доставка и оплата',
  CONTENT: [
    {
      type: 'text',
      content:
        'Для наших покупателей доступны 2 способа доставки: курьерская доставка и самовывоз.',
    },
    { type: 'subtitle', content: 'Курьерская доставка' },
    { type: 'text', content: 'Курьеры работают каждый день с 11 до 21 часа.' },
    {
      type: 'text',
      content:
        'Доставка покупок осуществляется только по городу в пределах кольцевой автомобильной дороги',
    },
    {
      type: 'text',
      content:
        'Если Вы готовы принять заказ в интервале с 12 до 17 часов, то доставка будет стоить 300 рублей. ',
    },
    { type: 'text', content: 'При заказе от 3000 рублей доставка БЕСПЛАТНАЯ.' },
    {
      type: 'text',
      content:
        'При оформлении заказа до 15 часов возможна доставка в тот же день в промежутке с 17 до 21 часа.',
    },
    { type: 'text', content: 'Курьер предупредит Вас о своём прибытии за 30-40 минут.' },
    { type: 'subtitle', content: 'Самовывоз' },
    {
      type: 'text',
      content:
        'Вы можете сделать заказ и забрать его самостоятельно как с точек самовывоза, так и непосредственно с нашего производства по адресу: ул. Абрикосовая, 25а, кафе «Сова». С актуальными адресами других точек самовывоза можно ознакомиться на странице Наши контакты',
    },
    {
      type: 'text',
      content:
        'Заказ необходимо сделать до 20 часов, чтобы забрать его на следующий день в пункте самовывоза в удобное для вас время в промежутке с 13 до 22 часов.',
    },
    { type: 'subtitle', content: 'Оплата:' },
    {
      type: 'text',
      content:
        'Оплатить заказ можно наличными или банковской картой при получении: курьеры оснащены современными мобильными терминалами.',
    },
  ],
  IMAGE: bicycle,
};

export const TERMS: additionalPagesData = {
  TITLE: 'Условия использования интернет-витрины «MACARON-SHOP»',
  STAMP: 'Вступает в силу 2 мая 2025 года',
  CONTENT: [
    {
      type: 'text',
      content:
        'Порядок использования интернет-витрины «MACARON-SHOP» (далее – «Сайт»), регулируется настоящими Условиями использования (далее – «Условия»), а также применимым законодательством.',
    },
    {
      type: 'text',
      content:
        'Для целей настоящих Условий под пользователем понимается лицо, имеющее доступ к Сайту посредством сети интернет и использующее Сайт после прохождения на нем регистрации и (или) авторизации, либо использующее Сайт без выполнения указанных действий (далее – «Пользователь»). ',
    },
    {
      type: 'text',
      content:
        'Используя Сайт, Пользователь соглашается с настоящими Условиями. В случае несогласия с Условиями Пользователь не должен использовать Сайт.',
    },
    { type: 'subtitle', content: 'Безопасность информационной системы:' },
    {
      type: 'text',
      content:
        'Пользователю запрещено нарушать безопасность информационной системы Сайта и (или) совершать действия, которые могут нарушить безопасность информационной системы Сайта. Нарушение безопасности информационной системы со стороны Пользователя влечет за собой ответственность в соответствии с применимым законодательством. ',
    },
    {
      type: 'text',
      content:
        'Все случаи возможного нарушения безопасности информационной системы Сайта со стороны Пользователя будут расследованы в сотрудничестве с компетентными органами.',
    },
    {
      type: 'subtitle',
      content:
        'Информация для родителей и законных представителей несовершеннолетних пользователей ',
    },
    {
      type: 'text',
      content:
        'Наш интернет-магазин предоставляет услуги пользователям от 13 лет и старше. Мы не несем ответственности за действия несовершеннолетних младше указанного возраста, которые получили неограниченный доступ к компьютеру или другим устройствам и самостоятельно совершили заказ. Ответственность за контроль и доступ детей к интернету и возможности оформления заказов лежит на их родителях или законных представителях.',
    },
    { type: 'subtitle', content: 'Контактные данные собственника Сайта:' },
    {
      type: 'text',
      content:
        'Пользователи Сайта могут направлять предложения, замечания, вопросы, имеющие отношение к настоящим Условиям пользования, на адрес электронной почты macaron-city@gmail.com',
    },
  ],
  IMAGE: pickup2,
};

export const PRIVACY: additionalPagesData = {
  TITLE: 'Политика конфиденциальности',
  STAMP: 'Вступает в силу 2 мая 2025 года',
  CONTENT: [
    {
      type: 'text',
      content:
        'Настоящая Политика конфиденциальности персональных данных (далее - Политика конфиденциальности) действует в отношении всей информации, размещенной на настоящем сайте в сети Интернет по адресу: https://macaron.ru/ (далее - Сайт), которую владельцы данного ресурса могут получить о посетителях, пользователях и иных лицах, вносящих сведения о себе в имеющиеся на Сайте поля для заполнения и в иных случаях, связанных с использованием указанными лицами Сайта, его сервисов, программ и продуктов. Использование сервисов Сайта означает безоговорочное согласие Пользователя с настоящей Политикой и указанными в ней условиями обработки его персональной информации; в случае несогласия с этими условиями Пользователь должен воздержаться от использования сервисов.',
    },
    {
      type: 'subtitle',
      content: 'Обработка персональных данных',
    },
    {
      type: 'text',
      content:
        'Мы собираем и обрабатываем персональные данные пользователей исключительно для обеспечения работы интернет-магазина. Данные используются для обработки заказов, доставки, связи с клиентами и предоставления необходимого сервиса.',
    },
    {
      type: 'text',
      content:
        'Мы гарантируем, что персональные данные не передаются третьим лицам без согласия пользователя, за исключением случаев, предусмотренных законодательством.',
    },
    {
      type: 'text',
      content:
        'Мы рекомендует всем Пользователям ответственно относиться к использованию сети Интернет и принимать меры по защите персональных данных и иной информации.',
    },
  ],
  IMAGE: pickup2,
};

export const CONTACTS: additionalPagesData = {
  TITLE: 'Контакты',
  CONTENT: [
    {
      type: 'subtitle',
      content: 'Производство',
    },
    {
      type: 'text',
      content: 'Санкт-Петербург, ул. Абрикосовая, 25а, кафе «Сова»',
    },
    {
      type: 'subtitle',
      content: 'Пункты самовывоза',
    },
    {
      type: 'text',
      content: 'Санкт-Петербург, Невский проспект, 13, ресторан «Амстердам»',
    },
    {
      type: 'text',
      content: 'Санкт-Петербург,  ул.Бонч-Бруевича, 3а, кафе-кондитерская «Минск» ',
    },
    {
      type: 'text',
      content: 'Белыничи, ул. Виноградная, кафе «Веселый кот»',
    },
    {
      type: 'subtitle',
      content: 'Телефоны',
    },
    {
      type: 'text',
      content: '+7 (812) 309-82-88 основной номер',
    },
    {
      type: 'text',
      content: '+375 (17) 199-82-88  для жалоб и предложений',
    },
  ],
  IMAGE: letter,
};

export const RETURNS: additionalPagesData = {
  TITLE: 'Отмена заказа и возврат',
  STAMP: 'Вступает в силу 2 мая 2025 года',
  CONTENT: [
    {
      type: 'text',
      content: 'Отменить заказ можно до 20.00 за день до даты заказа',
    },
    {
      type: 'text',
      content:
        'Мы уделяем особое внимание качеству наших изделий. Если при получении заказа вы обнаружите явный производственный брак пирожных, возврат или обмен изделия возможен. Для оперативного решения проблемы просим обратиться в службу поддержки.',
    },
  ],
  IMAGE: pickup1,
};
